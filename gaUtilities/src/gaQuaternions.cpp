...
namespace gaEngineSDK {
  ...
  Quaternions::Quaternions(const Radians& X, 
                           const Radians& Y, 
                           const Radians& Z, 
                           const Radians& W) {
  ...
  Quaternions::Quaternions(const Degrees& X, 
                           const Degrees& Y, 
                           const Degrees& Z,
                           const Degrees& W) {
    float fSinPitch = Math::fastSin(X.valueRadians() * 0.5f);
  ...
  Quaternions::getEuler() {
    ...
    euler.z = Math::arcTan(2 * (x * y + w * z), w * w + x * x - y * y - z * z);
    ...
    euler.x = Math::arcTan((2) * (y * z + w * x), w * w - x * x - y * y + z * z);
